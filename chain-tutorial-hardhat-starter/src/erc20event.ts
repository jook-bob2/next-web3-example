...
async function main() {
  ...
  const token: ClassToken = await ethers.getContractAt(
    "ClassToken",
    contractAddress
  );
  ...
  console.log(
    formatAddress(address0, 2),
    "give allowance to ",
    formatAddress(address1, 2),
    formatEther(allowance),
    "CLT"
  );

  console.log("==transferFrom 50.0==");
  txresponse = await token
    .connect(account1)
    .transferFrom(address0, address2, parseEther("70.0"));
  ...
  console.log(
    formatAddress(address0, 2),
    ":",
    formatEther(await token.balanceOf(address0)),
    " CLT"
  );
  console.log(
    formatAddress(address2, 2),
    ":",
    formatEther(await token.balanceOf(address2)),
    " CLT"
  );
